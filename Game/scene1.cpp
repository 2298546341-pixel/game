...
void RenderScene_StartScene(HDC hdc_memBuffer, HDC hdc_loadBmp)
{
	...
	// 选择自定义字体到设备上下文
	HFONT hOldTITLEFont = (HFONT)SelectObject(hdc_memBuffer, TITLEFont);
	// 设置字体区域
	const int TITLEwidth = 800;
	const int TITLEheight = 200;
    const int TITLEleft = WINDOW_WIDTH / 2 - TITLEwidth / 2 - 10;
	const int TITLEtop = 100;
	const int TITLEright = TITLEleft + TITLEwidth;
	const int TITLEbottom = TITLEtop + TITLEheight;
	RECT TITLErect = { TITLEleft, TITLEtop, TITLEright, TITLEbottom };
	// 绘制
	DrawText(hdc_memBuffer, TEXT("飞机大战"), -1, &TITLErect,
		DT_CENTER);
	// 恢复原来的字体
	SelectObject(hdc_memBuffer, hOldTITLEFont);
	// 删除自定义字体以释放资源
